...
def cached_normalized_search_texts(
    cache_key: tuple[object, ...],
    _poems: tuple[Poem, ...],
) -> tuple[str, ...]:
    _ = cache_key
    return tuple(
        normalize_search_text(poem_text(poem)) for poem in _poems
    )
...
@st.cache_resource(show_spinner=False)
def app_process() -> psutil.Process:
...
title_column, memory_column = st.columns([4, 2])
with title_column:
...
with st.sidebar:
    ...
    selected_authors = st.multiselect(
        ...
        args=("selected_authors", all_authors),
    )
